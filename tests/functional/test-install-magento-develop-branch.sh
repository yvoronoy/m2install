...
OUTPUT=$(${BIN_M2INSTALL} --force --source git -v 2.4-develop --ee 2>error.log)
...
OUTPUT=$(${BIN_M2INSTALL} --force --source git -v 2.4-develop --ee --es-host es --es-port 9202 2>error.log)
assertContains "$([ -f error.log ] && cat error.log)" "ElasticSearch is not available on es:9202." "Es is not available"

OUTPUT=$(${BIN_M2INSTALL} --force --source git -v 2.4-develop --ee --es-host magento2elastic7 --es-port 9207 2>error.log)
