...
[[ -z "$1" || "$1" = "play" ]] && sleep 5 && docker run --rm -it -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -v "$(pwd)/infra/ansible/etc:/etc/ansible" -v "$(pwd):/root/m2install" -w /root/m2install/infra/terraform terraform-runner ansible-playbook ../ansible/playbook.yaml
